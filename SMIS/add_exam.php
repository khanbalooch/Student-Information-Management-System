...
<form action="add_exam.php" method="post" name="form10"><br />
...
//	  echo $_POST[ass_id];  echo '<br/>';
	...
	$sql_insert="INSERT INTO exam (exam_id,course_id,sem_id,sec_name,exam_marks,exam_wtg,exam_date) VALUES($_POST[ass_id],'".$course_id."','".$semester_id."','$_POST[selectsection]',$_POST[ass_mk],$_POST[ass_wt],'$_POST[ass_d_date]')";
...
Exam No<input type="number" name="ass_id" value=""/><br/>
...
Date&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="ass_d_date" value="DD-MON-YYYY"/><br/>
...
<form action="add_exam.php" method="post" name="form15" >
...
if($_POST["show_all"]=='Show All'){
	...
	$sql_select="select q.exam_id,c.course_name,s.sem_id,q.sec_name,q.exam_marks,q.exam_wtg,q.exam_date
	 from exam q,course c,semester s,section sc
	...
	while($row = oci_fetch_array($selectquery, OCI_BOTH+OCI_RETURN_NULLS))  //parse or execute for update, insert
	{
		...
		//echo 'fetching_array';
		echo '<tr>
		<td scope="col">'.$row["EXAM_ID"].'&nbsp;</td>
		<td scope="col">'.$row["COURSE_NAME"].'&nbsp;</td>
		<td scope="col">'.$row["SEM_ID"].'&nbsp;</td>
		<td scope="col">'.$row["SEC_NAME"].'&nbsp;</td>
		<td scope="col">'.$row["EXAM_MARKS"].'&nbsp;</td>
		<td scope="col">'.$row["EXAM_WTG"].'&nbsp;</td>
		<td scope="col">'.$row["EXAM_DATE"].'&nbsp;</td>
		</tr>';
		}
		echo '</table>';
