...
<form action="add_quiz.php" method="post" name="form10"><br />
...
if($_POST["update"]=='Update')
   {
	...
	$sql_insert="INSERT INTO quiz (quiz_id,course_id,sem_id,sec_name,quiz_marks,quiz_wtg,quiz_date) VALUES($_POST[ass_id],'".$course_id."','".$semester_id."','$_POST[selectsection]',$_POST[ass_mk],$_POST[ass_wt],'$_POST[ass_d_date]')";
...
<form action="add_quiz.php" method="post" name="form15" >
...
if($_POST["show_all"]=='Show All'){
	...
	$sql_select="select q.quiz_id,c.course_name,s.sem_id,q.sec_name,q.quiz_marks,q.quiz_wtg,q.quiz_date
	 from quiz q,course c,semester s,section sc
	...
	while($row = oci_fetch_array($selectquery, OCI_BOTH+OCI_RETURN_NULLS))  //parse or execute for update, insert
	{
		...
		//echo 'fetching_array';
		echo '<tr>
		<td scope="col">'.$row["QUIZ_ID"].'&nbsp;</td>
		<td scope="col">'.$row["COURSE_NAME"].'&nbsp;</td>
		<td scope="col">'.$row["SEM_ID"].'&nbsp;</td>
		<td scope="col">'.$row["SEC_NAME"].'&nbsp;</td>
		<td scope="col">'.$row["QUIZ_MARKS"].'&nbsp;</td>
		<td scope="col">'.$row["QUIZ_WTG"].'&nbsp;</td>
		<td scope="col">'.$row["QUIZ_DATE"].'&nbsp;</td>
		</tr>';
		}
		echo '</table>';
